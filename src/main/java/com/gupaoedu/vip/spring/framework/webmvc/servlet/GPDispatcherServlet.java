...
public class GPDispatcherServlet extends HttpServlet {
    ...
    /**
     * 读取所有beanDefnitionReader，配置文件生成扫描所有类的RequestMapping注解
     * 一般在controller类。然后在顶一个HandlerMapping类，保存三个属性，第一个属性，这个地址的处理的类实例，
     * 第二个属性是方法，第三个属性是正则表达式。用来查询，然后注册完成一个List
     */
    private List<GPHandlerMapping> handlerMappings = new ArrayList<GPHandlerMapping>();

    /**
     * 存储所有的以url所对应的Key为GPHandlerMapping，所对的ValueGpHandlerAdater 执行
     */
    ...
    public void init(ServletConfig config) throws ServletException {
        // 第一 IOC容器初始化了
        GPApplicationContext context = new GPApplicationContext(new String[]{config.getInitParameter(LOCATION)});
        // 第二 MVC初始化
    ...
    /**
     * 第一步，handlerMapping只是将所有url一一对应，但是方法的运行，还是需要参数，
     * handlerAdapter适配器适配的就是方法运行的参数进行组装
     *
     * @param context
     */
    ...
    private void doDispatch(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        GPHandlerMapping gpHandlerMapping = getHandler(req);
        if (gpHandlerMapping == null) {
        ...
        GPHandlerAdapter handlerAdapter = getHandlerAdapter(gpHandlerMapping);

        // 第三步，根据handlerAdapter.执行方法
        GPModelAndView gpModelAndView = handlerAdapter.handle(req, resp, gpHandlerMapping);
    ...
    private void processDispatchResult(HttpServletResponse resp, GPModelAndView gpModelAndView) throws Exception {
        ...
        for (GPViewResolver viewResolver : this.viewResolvers) {
            ...
            if (!gpModelAndView.getViewName().equals(viewResolver.getViewName())) {
                continue;
            }
    ...
    /**
     * 根据handlerMapping获取handlerAdapter 专人干专事
     *
     * @param gpHandlerMapping
     */
